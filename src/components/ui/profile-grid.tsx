...
const ProfileGrid: React.FC<ProfileGridProps> = ({ user }) => {
  ...
  const styles: Record<string, string> = {
    experience:
      "bg-[#e2f0cb] dark:text-black border rounded shadow p-2 drag-handle cursor-move",
    skill:
      "bg-[#fcdce1]  dark:text-black border rounded shadow p-2 drag-handle cursor-move",
    role: "bg-[#c8ceee]  dark:text-black border rounded shadow p-2 drag-handle cursor-move",
  };
            ...
            case "experience":
              content = (
                <PortfolioEntryExperience
                  experience={experiences[info.foreignId]}
                />
              );
              break;
            case "skill":
